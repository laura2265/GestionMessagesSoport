...
/* =========================================
   API colección conversacion-server (Local)
   ========================================= */
...
function MessageChat() {
  const notifiedMessagesRef = useRef(new Map()); // Map<convKey, Set<idMessage>>
  const audioRef = useRef(new Audio(Notificacion));
  const [notificacions, setNotificacions] = useState([]);
  const [audioEnabled, setAudioEnabled] = useState(true);
  const pendienteConversacion = useRef(new Map()); // Map<convKey, payload POST message/>
      ...
      try {
        ...
        for (let user of assignedEmple) {
          ...
          if (chatuser !== 'local') {
            ...
            continue; // fin canal externo (con posible fallback)
          }
